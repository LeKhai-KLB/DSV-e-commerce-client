...
function AdminProducts() {
    ...
    const reLoadProductList = async () => {
        ...
        const queryString = `?searchValue=${searchValue}&sortValue=${JSON.stringify(sortValue)}&slice=${first}-${currentMaxShow}`
        try {
            const {data} = await axios.get(process.env.REACT_APP_GET_PRODUCT_BY_FILTER_AND_SORT_VALUE_API + queryString)
            if(data.status === 200) {
                setProductListSlice(data.resultData.products)
    ...
    return (
        <div className={styles.adminProductsContainer} >
            ...
            <div className={styles.entriesContainer} >
                ...
                {/* pagination */}
                <PaginationBar entriesQty={recordCount} maxShowEntries={10} onChangePage={setCurrentPage} onchangeMaxShow={setCurrentMaxShow}/>
            </div>
        </div>
    )
}
