...
import DatetimeRangePicker from '../../components/AdminComponent/DatetimeRangePicker'
...
function AdminOrders() {
    ...
    const handleSetState = async (state, title, index, id) => {
        const stateBox = document.getElementById('stateBox' + index)
        if(title !== stateBox.innerText) {
            try {
                ...
                if(data.status === 200) {
                    stateBox.innerText = title
                    if(title === 'Completed') {
                        if(stateBox.classList.contains(styles.yellow))
                            stateBox.classList.remove(styles.yellow)
                        else 
                            stateBox.classList.remove(styles.red)
                        stateBox.classList.add(styles.green)
                    }
                    else if(title === 'Canceled') {
                        if(stateBox.classList.contains(styles.yellow))
                            stateBox.classList.remove(styles.yellow)
                        else 
                            stateBox.classList.remove(styles.green)
                        stateBox.classList.add(styles.red)
                    }
                }
                else {
                                ...
                                {/* actions box */}
                                {
                                    ...
                                    <td className={styles.actionsBox}>
                                        ...
                                        <div 
                                            ...
                                            onClick={() => handleSetState(2, 'Completed', i + 1, e?._id)}
                                        >
                                        ...
                                        <div 
                                            className={styles.actionButton}
                                            onClick={() => handleSetState(3, 'Canceled', i  + 1, e?._id)}
                                        >
