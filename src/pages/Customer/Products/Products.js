...
import FilterDropdownBar from "../../../components/Customer/FilterBox/FilterDropdownBar";
...
import FilterByWordBox from "../../../components/Customer/FilterBox/FilterByWordBox";
import FilterPriceBox from "../../../components/Customer/FilterBox/FilterPriceBox";
import SortBox from "../../../components/Customer/SortBox";
...
function Products() {
  ...
  const handleLoadProductList = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        process.env.REACT_APP_GET_PRODUCT_BY_FILTER_AND_SORT_VALUE_API +
          "?" +
          queryString
      );
      if (data.status === 200) {
  ...
  const handleLoadBrandList = async () => {
    try {
      const categoryId = queries.get("category")
        ? queries.get("category")
        : params.seRankCategory.split("_")[1];
      const brandData = await axios.get(
        process.env.REACT_APP_GET_ALL_BRANDS_PASS_CATEGORY_API +
          "?category=" +
          categoryId
      );
  ...
  const handleFirstLoad = async () => {
    try {
      ...
      const categoryData = await axios.get(
        process.env.REACT_APP_GET_CATEGORY_PASS_BY_PARENT_VALUE_API +
          "?parent=" +
          parentId
      );
      if (categoryData.data.status === 200) {
        setCategoryList([
          { name: "All " + params.seRankCategory.split("_")[0], _id: parentId },
          ...categoryData.data.resultData,
        ]);
      }
  ...
  useEffect(() => {
    ...
    if (Object.keys(temp).length !== 0) handleLoadProductList();
  }, [queries]);
  ...
  return (
    <div className={styles.productsPageContainer}>
      <div className={styles.pageTitleContainer}>
        {params.fstRankCategory +
          " / " +
          params.seRankCategory.split("_")[0].replace("+", " ")}
      </div>
      <div className={styles.contentContainer}>
        ...
        <div className={styles.handleSideContainer}>
              ...
              categoryList.map((e, i) => (
                <span
                  ...
                  className={`${styles.categorySubTitle} ${
                    queries.get("category")
                      ? queries.get("category") === e._id
                        ? styles.highlight
                        : ""
                      : params.seRankCategory.split("_")[1] === e._id
                      ? styles.highlight
                      : ""
                  }`}
          ...
          <div className={styles.filterBox}>
            <span className={styles.boxTitle + " " + styles.mtb_5}>Filter</span>
            ...
            <FilterDropdownBar title={"Available"}>
              <FilterByWordBox
                ...
                title="available"
                value={
                  queries.get("available")
                    ? queries.get("available").split("-")
                    : null
                }
        ...
        <div className={styles.productsContainer}>
          ...
          <div className={styles.topProductsContainer}>
            ...
            <Pagination
              count={
                Math.ceil(Number(totalLength) / 20) === 0
                  ? 1
                  : Math.ceil(Number(totalLength) / 20)
              }
              boundaryCount={1}
              page={queries.get("page") ? Number(queries.get("page")) : 1}
          ...
          <div className={styles.middleProductsContainer}>
              ...
              {productList.length !== 0 ? (
                productList.map((p, i) => (
                  <div
                    ...
                    onClick={() => handleClickProductCard(p._id)}
                  >
                    <img
                      ...
                      alt={p.name}
                    />
                    <div title={p.name} className={styles.cardTitle}>
                      {p.name}
                    </div>
            ...
            {isLoading && (
              <div
                className={styles.gridContainer + " " + styles.overLayContainer}
              >
