  ...
  return (
    <div className={styles.profileContainer}>
      ...
      <div className={styles.tabContentContainer}>
        {currentTab === 0
          ? user && <AccountSettingTab user={user} />
          : user && (
              <ChangePasswordTab user={user} onChangeTab={setCurrentTab} />
            )}
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className={styles.taskBar}>
        <span className={`${styles.typography} ${styles.bold}`}>
          Infomation
        </span>
        <span
    ...
    <form className={styles.viewForm}>
      ...
      <div className={styles.contentContainer}>
        <div className={styles.fieldsContainer}>
          ...
          {/* name field */}
          <span className={`${styles.typography} ${styles.fieldMargin}`}>
            {user?.userName}
          </span>
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await axios.post(
        process.env.REACT_APP_UPDATE_USER_INFO,
        {
          _id: user.id,
          userName: data.name,
          email: data.email,
