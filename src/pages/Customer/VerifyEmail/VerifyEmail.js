...
const status = {
    ...
    warning: {
        ...
        description: 'Your email had been verified or this mail had been expired.\n You are being redirected to home page, it may takes upto',
        countDown: 5
    }
}
...
function VerifyEmail() {
    ...
    const [countDown, setCountDown] = useState(currentStatus.countDown ? currentStatus.countDown: -1)
    ...
    const handleLoad = async () => {
        ...
        if(token) {
            try {
                const { data } = await axios.post(process.env.REACT_APP_VERIFY_EMAIL, {}, {
                    headers: {
    ...
    useEffect(() =>{
        if(currentStatus.countDown) {
            interValId.current = setInterval(() => 
                setCountDown(prev => {
                    if(prev === -1)
                        return currentStatus.countDown
                    else
                        return --prev
                })
            ,1000)
        ...
        <div className={styles.verifyEmailContainer}>
            <div className={styles.messageBox} >
                ...
                <div className={styles.textWrapper} >
                    ...
                    <span className={styles.description}>{currentStatus.description}</span>
                    {
                        countDown !== -1 &&
