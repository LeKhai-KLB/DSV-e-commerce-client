...
function Product() {
    ...
    const [sameNestedRootCategoryProducts, setSameNestedRootCategoryProducts] = useState([])
    ...
    const handleFirstLoad = async () => {
        try {
            ...
            if(data.status === 200) {
                ...
                setMainImage(0)
                setQuantity(1)
                handleLoadSameBrandProducts(data.resultData.brand._id, data.resultData._id)
                handleLoadSameParentCategory(data.resultData.categories[0].parent, data.resultData._id)
            }
            else {
                throw new Error(data.errorMessage)
            }
            ...
            <div className={styles.contentContainer} >
                ...
                <div className={styles.topContainer} >
                    ...
                    <div className={styles.subImagesContainer} >
                        {
                            ...
                            [...Array(4)].map((e, i) => {
                                if(images[i])
                                    ...
                                    return (<div key={i} className={styles.noneImage} />)
                            })
                        }
                    </div>
                    ...
                    <div className={styles.productInfoContainer} >
                        ...
                        <div title={product ? product.name:'Unknown'} className={styles.productInfoTitle} >
                        ...
                        <div className={styles.reviewsBox} >
                            <div className={styles.starsBox}> 
                                {
                                    [...Array(5)].map((e, i) => (
                                        <img key={i} className={styles.startIcon} src={startIcon} alt = ' ' />
                                    ))
                                }
                        ...
                        <div className= {styles.flexBox} >
                            ...
                            <div className={styles.quantityField} >
                                <img 
                                    className= {`${styles.largeIcon} ${quantity === 1 || !Number(quantity) ? styles.inactive:''}`} 
                                    src={minusIcon} 
                                    alt=' ' 
                                ...
                                <input 
                                    ...
                                    onBlur={(e) => {
                                        if(Number(e.target.value) >= 1 && Number(e.target.value) <= (product ? product?.inStock[size]:1))
                                            setQuantity(e.target.value)
                                        else 
                                            if(Number(e.target.value) < 1)
                                                setQuantity(1)
                                            else if(Number(e.target.value) > (product ? product?.inStock[size]:1)) {
                                                setQuantity(product ? product?.inStock[size]:1)
                                            }
                                    }}
                                />
                                <img 
                                    className= {`${styles.largeIcon} ${product && product.inStock[size] > quantity ? '':styles.inactive}`} 
                                    src={plusIcon} 
                                    alt=' ' 
                    ...
                    <div className={styles.sameBrandContainer} >
                        ...
                        <div title={product ? product.brand.name:'Unknown'} className={styles.sameBrandName} >{product ? product.brand.name:'Unknown'}</div>
                                ...
                                [...Array(4)].map((e, i) => {
                                    if(sameBrandProducts[i])
                                        return (<img 
                                                    ...
                                                    onClick={() => nav('../product/' + sameBrandProducts[i]._id)}
                                                    alt=' '
                                                />)
                                    else 
                                        return (<div key={i} className={styles.noneImage} />)
                ...
                <div className={styles.reviewsContainer} >
                    <div className={styles.containerTitle}>Reviews</div>
                </div>
                            ...
                            if(sameNestedRootCategoryProducts[i])
                                return (
                                    <div 
                                        ...
                                        onClick={() => nav('../product/' + sameNestedRootCategoryProducts[i]._id)}
                                    >
                                        <img 
                                            ...
                                            alt =' ' 
                                        />
                                        <div className={styles.sameCategoryTitle}>{sameNestedRootCategoryProducts[i].name}</div>
                                    </div>
                                )
                            else
                                return (
                                    <div key={i} className={styles.noneImageM} />
                                )
                        })
                    }
